...
public class ElitePaws extends Monster implements IAnimatable {
    ...
    public ElitePaws(EntityType<ElitePaws> type, Level world) {
        super(type, world);
    }

    protected ElitePaws(Level world) {
        this(EntityInit.ELITE_PAWS.get(), world);
    }
    ...
    public void handleEntityEvent(byte id) {
        if (id == 10) {
            ...
            controller.setAnimation(new AnimationBuilder().addAnimation("animation.elite_paws.attack"));
        } else
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.elite_paws.walk", true));
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.elite_paws.attack", false));
            this.swinging = false;
        }
