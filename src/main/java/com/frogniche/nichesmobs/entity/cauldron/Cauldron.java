...
public class Cauldron extends Monster implements IAnimatable {

    public static final String CONTROLLER_NAME = "controller1";

    protected static final EntityDataAccessor<Boolean> SLEEP = SynchedEntityData.defineId(Cauldron.class, EntityDataSerializers.BOOLEAN);
    protected static final EntityDataAccessor<Boolean> WAS_SLEEP = SynchedEntityData.defineId(Cauldron.class, EntityDataSerializers.BOOLEAN);
    @OnlyIn(Dist.CLIENT)
    ...
    public Cauldron(EntityType<Cauldron> type, Level level) {
        super(type, level);
    }
    ...
    public void handleEntityEvent(byte id) {
        if (id == 10) {
            AnimationController<Cauldron> controller = GeckoLibUtil.getControllerForID(this.factory, this.getId(), CONTROLLER_NAME);
            controller.setAnimation(new AnimationBuilder().addAnimation("animation.n_cauldron.attack"));
        } else
            super.handleEntityEvent(id);
    }

     private PlayState predicate(AnimationEvent<Cauldron> event){
