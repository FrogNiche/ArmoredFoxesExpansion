...
public class SawPaws extends Monster implements IAnimatable {
    ...
    public SawPaws(EntityType<? extends Monster> p_33002_, Level p_33003_) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new NearestAttackableTargetGoal(this, Runt.class, true));
        ...
        this.goalSelector.addGoal(11, new RandomStrollGoal(this, 0.5f, 10));
    ...
    private PlayState attackPredicate(AnimationEvent<SavagerGuardEntity> event) {
        if (this.swinging && event.getController().getAnimationState() == AnimationState.Stopped) {
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.saw_paws.ATTACK", false));
            this.swinging = false;
        }
    ...
    private PlayState predicate(AnimationEvent<SavagerGuardEntity> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.saw_paws.walk", true));
            return PlayState.CONTINUE;
        } else
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.saw_paws.idle", true));
        return PlayState.CONTINUE;
    }
