...
public class Sorcerer extends Monster implements IAnimatable {
    ...
    public Sorcerer(EntityType<Sorcerer> type, Level world) {
        super(type, world);
    }

    protected Sorcerer(Level world) {
        this(EntityInit.SORCERER.get(), world);
    }
    ...
    public void handleEntityEvent(byte id) {
        if (id == 10) {
            ...
            controller.setAnimation(new AnimationBuilder().addAnimation("animation.sorcerer.attack"));
        } else
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.sorcerer.walk", true));
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.sorcerer.attack", false));
            this.swinging = false;
        }
